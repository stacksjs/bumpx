...
/**
 * Extract GitHub repository owner and name from a git remote URL
 */
export function extractGitHubRepoInfo(remoteUrl: string): { owner: string, repo: string } | null {
  try {
    ...
    // Handle git protocols: git://github.com/owner/repo.git
...
export async function createGitHubRelease(
  ...
  try {
    // Try to dynamically import Octokit
    const { Octokit } = await import('@octokit/rest')
    const octokit = new Octokit({ auth: options.token })

    // Get repository owner and name if not provided
    let { owner, repo } = options
    if (!owner || !repo) {
      try {
        // Get the remote URL
        const remoteUrl = executeGit(['config', '--get', 'remote.origin.url'], options.cwd)
        const repoInfo = extractGitHubRepoInfo(remoteUrl)

        if (!repoInfo) {
          throw new Error('Could not determine GitHub repository from git remote URL')
        }
        ...
        throw new Error(`Failed to get GitHub repository information: ${error}`)
      }
    }

    // Get release body content from changelog if not provided
    let body = options.body
    if (!body && options.changelogPath) {
      try {
        const fs = await import('node:fs')
        const path = await import('node:path')
        const changelogPath = path.resolve(options.cwd || process.cwd(), options.changelogPath)

        if (fs.existsSync(changelogPath)) {
          const changelogContent = fs.readFileSync(changelogPath, 'utf-8')

          // Extract the latest release section from the changelog
          const releasePattern = new RegExp(`## \\[?v?${tagName.replace(/^v/, '')}\\]?.*?(?:## |$)`, 's')
        ...
        console.warn(`Warning: Failed to read changelog for release body: ${error}`)
      }
    }
